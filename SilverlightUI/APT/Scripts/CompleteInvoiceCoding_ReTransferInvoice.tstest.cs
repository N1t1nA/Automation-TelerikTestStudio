...
namespace APT.Scripts
{
    ...
    public class CompleteInvoiceCoding_ReTransferInvoice : BaseWebAiiTest
    {
       ...
       //@Developed By: nitinahu          
        ...
        public CompleteInvoiceCoding_ReTransferInvoice()
        {           
            //Read the Data Row from XML 
            drAPT = objReadXml.Read_xml_file("APT_TestData.xml", "CompleteInvoiceCodingReTransferInvoice", "TestCase", "88878");        
        }      
        ...
        [CodedStep("Select 'Incomplete for transfer' Invoices From Overview", RequiresSilverlight = true)]
        public void SelectIncompleteForTransferInvoice()
        { 

            try
            {
                //Method to Open Draft Invoice from Overview
                p2pInvoiceAdministrationInvoiceDetailsObj.P2PInvoiceAdministrationOverviewInvoiceStatusesReceived(drAPT["InvoiceStatus"].ToString(), drAPT["IncompleteForTransferInvoiceCountifDisabled"].ToString());
            }
             catch (Exception selectIncompleteForTransferInvoice)
            {
                Globals.testScriptName = ExecutionContext.Test.Name + "_";
                exceptionHandler.CapturedImage(selectIncompleteForTransferInvoice, Globals.testScriptName);
            }
        ...
        [CodedStep("Search The 'Incomplete for transfer' Invoice 'Auto_Incomplete_BW80_0'", RequiresSilverlight = true)]
        public void SearchIncompleteForTransferInvoiceNumber()
        {

            try
            {
                //Call the Method to Search for Invoice
                p2pPaymentPlansSearchObj.P2PPaymentPlansFreeTextSearch(drAPT["IncompleteForTransferInvoiceNumber"].ToString(), drAPT["IncompleteForTransferInvoiceNumber"].ToString());
            }
        ...
        [CodedStep("Open 'Incomplete for transfer' Invoice 'Auto_Incomplete_BW80_0'", RequiresSilverlight = true)]
        public void OpenInvoice()
        {            
            //Create an object for shared class
            P2PInvoiceAdministrationToolbarActions openSelected = new P2PInvoiceAdministrationToolbarActions(ActiveBrowser);
            try
            {
                //Call the Method to Open the Selected Invoice         
                openSelected.P2PInvoiceAdministrationOpenSelectedInvoice();
            }
        ...
        [CodedStep("Search the Invoice 'Auto_Incomplete_BW80_0' on Received Silo and Open Selected", RequiresSilverlight = true)]
        public void SearchInvoiceOnReceivedSilo()
        {
            try
            {
                //Use the Object to Access the Method for Search the Invoice
                searchObj.P2PInvoiceAdministrationFreeTextSearchToSearchInvoice(drAPT["IncompleteForTransferInvoiceNumber"].ToString());
            }
        ...
        [CodedStep("Add Coding Row CostCenter as 7810 and Net total as 40", RequiresSilverlight = true)]
        public void AddCodingRowDetailsView()
        {
            try
            {
            //Method to Add Coding Row in Details View
            p2pInvoiceAdministrationInvoiceDetailsObj.P2PInvoiceAdministrationDetailsAddCodingRow(Convert.ToInt32(drAPT["RowNumber"].ToString()), Convert.ToInt32(drAPT["CellNumber"].ToString()),drAPT["ContextMenu"].ToString(),drAPT["CostCenterCode"].ToString(), Convert.ToDouble(drAPT["Net_Sum"], CultureInfo.InvariantCulture.NumberFormat));
            }        
            catch (Exception addCodingRowDetailsView)
            {
                Globals.testScriptName = ExecutionContext.Test.Name + "_";
                exceptionHandler.CapturedImage(addCodingRowDetailsView, Globals.testScriptName);
            }
            //Call Coded Step HandleBusyIndicator
            HandleBusyIndicator();        
        }
        ...
        [CodedStep("Search The Updated Invoice 'Auto_Incomplete_BW80_0'", RequiresSilverlight = true)]
        ...
        public void VerifyInvoiceStatus()
        {
            try
            {
                ...
                p2pInvoiceAdministrationVerificationObj.P2InvoiceAdministration_Search_VerifyInvoiceStatus(drAPT["IncompleteForTransferInvoiceNumber"].ToString(), drAPT["VerifyFinalStatus"].ToString(), drAPT["Header_Name_Status"].ToString(), drAPT["Button_Text"].ToString());
            }
